...
function processFile(filePath) {
    fs.readFile(filePath, 'utf8', (err, data) => {
        if (err) {
            ...
            // Replace static class attributes only
            const newData = data.replace(/class\s*=\s*"(.*?)"/g, (match, p1) => {
                if (match.includes(':class')) {
                    return match; // Skip dynamic :class bindings
                }
                const fixedClassNames = p1.replace(/\s+/g, ' ').trim();
                return `class="${fixedClassNames}"`;
            });
            if (newData !== data) {
...
function startProcessing(directoryPath) {
    console.log(`\x1b[33mStarting to process Vue files...\x1b[0m`); // Yellow color
    traverseDirectory(directoryPath);

    process.on('exit', () => {
        console.log(`\n\x1b[36mTotal Vue files processed: ${totalFilesProcessed}\x1b[0m`);
